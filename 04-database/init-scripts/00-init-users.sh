...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "mytrader_dev" <<-EOSQL
    ...
    -- Permissões futuras para tabelas criadas POR mytrader_app
    -- Garante que mytrader_app mantém acesso total às próprias tabelas
    ...
    -- Permissões futuras para tabelas criadas por postgres
    ALTER DEFAULT PRIVILEGES IN SCHEMA public
        GRANT SELECT ON TABLES TO mytrader_readonly;

    -- ⭐ CHAVE: Permissões futuras para tabelas criadas POR mytrader_app
    -- Garante que quando mytrader_app executa migrations, mytrader_readonly
    -- recebe automaticamente permissões de SELECT
    ALTER DEFAULT PRIVILEGES FOR ROLE mytrader_app IN SCHEMA public
        GRANT SELECT ON TABLES TO mytrader_readonly;

    ALTER DEFAULT PRIVILEGES FOR ROLE mytrader_app IN SCHEMA public
        GRANT USAGE, SELECT ON SEQUENCES TO mytrader_readonly;
