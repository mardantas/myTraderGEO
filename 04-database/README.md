...
- ❌ Avoids false positives from SQLite in-memory (different SQL dialect)

### Implementation Files

The test fixtures are implemented in the IntegrationTests project:

| File | Purpose |
|------|---------|
| [Fixtures/DatabaseFixture.cs](../02-backend/tests/MyTraderGEO.IntegrationTests/Fixtures/DatabaseFixture.cs) | Base fixture that starts TestContainers, applies migrations and seeds |
| [Fixtures/DatabaseCollection.cs](../02-backend/tests/MyTraderGEO.IntegrationTests/Fixtures/DatabaseCollection.cs) | Collection definition for sharing fixture across test classes |

### Usage in Tests

```csharp
// Single test class with its own fixture
...
// Multiple test classes sharing same fixture (recommended for performance)
[Collection("Database")]
...
{
    private readonly DatabaseFixture _fixture;
    public StrategyRepositoryTests(DatabaseFixture fixture) => _fixture = fixture;
}
```
...
dotnet test 02-backend/tests/MyTraderGEO.IntegrationTests/ --logger "console;verbosity=detailed"
```

### Key Principles

- ✅ **SQL migrations BEFORE tests** - Fixture applies DBA scripts automatically
- ✅ **Real PostgreSQL** - Same version as production (15-alpine)
- ✅ **Idempotent migrations** - Safe to re-execute
- ✅ **Database-First** - Tests validate against actual SQL schema
